...
namespace StagPoint.Core
{
	...
	public class ListEx<T> : IList<T>
	{
		...
		internal ListEx( IList<T> listToClone )
			: this()
		{
			AddRange( listToClone );
		}

		internal ListEx( int capacity )
			: this()
		{
		...
		public ListEx<T> Clone()
		{
			...
			var clone = new ListEx<T>( this.count );
		...
		/// <summary>
		/// Removes elements (if necessary) to ensure that the list does not exceed <paramref name="Size"/> elements
		/// </summary>
		/// <param name="Size"></param>
		public void ShrinkTo( int Size )
		{
			RemoveRange( Size - 1, this.count - Size );
		}
		...
		/// <summary>
		/// Returns a subset of the collection's items as another ListEx
		/// </summary>
		public ListEx<T> GetRange( int index, int length )
		{
			var range = new ListEx<T>( length );
		...
		/// <summary>
		/// Returns a <see cref="ListEx"/> list containing all elements
		/// of the collection matching the condition specified by <paramref name="predicate"/>
		/// </summary>
		public ListEx<T> Where( Func<T, bool> predicate )
		{

			var result = new ListEx<T>( this.count );
		...
		/// </summary>
		public ListEx<TResult> Select<TResult>( Func<T, TResult> selector )
		{

			var result = new ListEx<TResult>( this.count );
		...
		/// </summary>
		public ListEx<T> Concat( ListEx<T> list )
		{

			var result = new ListEx<T>( this.count + list.count );
		...
		/// </summary>
		public ListEx<TResult> Convert<TResult>()
		{
		...
		/// than as a <see cref="ListEx"/>. For instance, LINQ contains 
		...
		private class PooledEnumerator : IEnumerator<T>, IEnumerable<T>
		{
			...
			public static PooledEnumerator Obtain( ListEx<T> list, Func<T, bool> predicate )
			{
