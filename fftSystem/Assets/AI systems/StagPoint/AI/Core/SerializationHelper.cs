			...
			foreach( Assembly assembly in System.AppDomain.CurrentDomain.GetAssemblies() )
			{
				foreach( System.Type type in assembly.GetTypes() )
				{
					...
					if( isMappable )
					{
						...
						var renameAttributes = type.GetCustomAttributes( typeof( TypeRenameAttribute ), false ) as TypeRenameAttribute[];
						for( int i = 0; i < renameAttributes.Length; i++ )
						{
							var attribute = renameAttributes[ i ];
							//Debug.LogWarning( string.Format( "Serialization - {0} will be deserialized as {1}", attribute.TypeName, type.FullName ) );
							typeCache[ attribute.TypeName ] = type;
						}
