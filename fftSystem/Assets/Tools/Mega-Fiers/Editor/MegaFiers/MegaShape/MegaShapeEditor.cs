...
public class MegaShapeEditor : Editor
{
	...
	public void OnSceneGUI()
	{
		...
		for ( int s = 0; s < shape.splines.Count; s++ )
		{
			...
			{
				Vector3 pp = shape.splines[s].knots[p].p;	//shape.transform.TransformPoint(shape.splines[s].knots[p].p);
				...
				if ( shape.drawKnots )	//&& recalc == false )
				{
					...
					pm = shape.splines[s].knots[p].p;	//shape.transform.TransformPoint(shape.splines[s].knots[p].p);
				...
				if ( shape.drawHandles )	//&& recalc == false )
				{
					...
					Vector3 ip = shape.splines[s].knots[p].invec;	//shape.transform.TransformPoint(shape.splines[s].knots[p].invec);
					Vector3 op = shape.splines[s].knots[p].outvec;	//shape.transform.TransformPoint(shape.splines[s].knots[p].outvec);
					Handles.DrawLine(pm, ip);	//shape.transform.TransformPoint(shape.splines[s].knots[p].invec));
					Handles.DrawLine(pm, op);	//shape.transform.TransformPoint(shape.splines[s].knots[p].outvec));
					...
					Vector3 hp = shape.splines[s].knots[p].invec;	//shape.transform.TransformPoint(shape.splines[s].knots[p].invec);
					//Handles.CubeCap(0, hp, Quaternion.identity, shape.KnotSize);
					if ( selected == p )
						Handles.Label(hp, " " + p);

					hp = shape.splines[s].knots[p].outvec;	//shape.transform.TransformPoint(shape.splines[s].knots[p].outvec);
					//Handles.CubeCap(0, hp, Quaternion.identity, shape.KnotSize);
